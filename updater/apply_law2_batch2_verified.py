#!/usr/bin/env python3
"""Prepare reviewed Law 2 batch 2 using the established guarded ledger."""
...
EVIDENCE=ROOT/'updater'/'law2-batch2-evidence.json'
...
for item in items:
    ...
    if norm(row.get('ground'))!=norm(item['ground']) or str(row.get('postcode') or '').strip().upper()!=item['postcode'].upper():
        raise SystemExit(f'ABORT: reviewed record drift for {club}: {row.get("ground")} {row.get("postcode")}')
...
print('LAW 2 BATCH 2 PREPARATION: SUCCESS')
