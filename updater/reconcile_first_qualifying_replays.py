...
the archived First Qualifying ties and its canonical first leg must already be
recorded as a draw. A diagnostic report is always emitted so guarded failures can
be inspected without weakening the publication rules.
"""
import json
import re
...
REPORT = ROOT / "updater" / "replay-reconciliation-report.json"
...
def pair_key(a, b): return tuple(sorted((norm(a), norm(b))))
...
def fixture_values(src): return list(src.values()) if isinstance(src, dict) else list(src or [])

def write_report(stage, **details):
    payload = {"checked_at": datetime.now(timezone.utc).isoformat(), "stage": stage, **details}
    REPORT.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")

def fail(stage, message, **details):
    write_report(stage, status="FAIL", message=message, **details)
...
def all_results(data):
    ...
    for rows in (data.get("result_history") or {}).values():
        if isinstance(rows, list): sources.extend(rows)
    for r in sources:
        if not isinstance(r, dict): continue
        ident = (norm(r.get("home")), norm(r.get("away")), r.get("date", ""), score_int(r.get("home_score")), score_int(r.get("away_score")), str(r.get("round") or "").lower())
        if ident not in seen: seen.add(ident); out.append(r)
    return out
...
def archived_ties(data):
    ...
    if len(ties) != EXPECTED_TIES:
        fail("archived_ties", f"Replay reconciliation blocked: expected {EXPECTED_TIES} archived First Qualifying ties, found {len(ties)}", archived_tie_count=len(ties), sample=ties[:5])
    return {pair_key(f["home"], f["away"]): f for f in ties}


def first_leg_is_draw(key, rows):
    for r in rows:
        if pair_key(r.get("home"), r.get("away")) != key: continue
        if "replay" in str(r.get("round") or "").lower(): continue
        hs, aw = score_int(r.get("home_score")), score_int(r.get("away_score"))
        if hs is not None and aw is not None and hs == aw: return True
    return False


def result_from_manifest(item):
    date, home, hs, aw, away, decision = item
    return {"home":home,"away":away,"home_score":hs,"away_score":aw,"winner":home if hs>aw else away,"status":"AET" if decision=="aet" else "FT","decision":decision,"date":date,"round":REPLAY_ROUND,"source_url":SOURCES[date]}


def same_result(a, b):
    return norm(a.get("home"))==norm(b.get("home")) and norm(a.get("away"))==norm(b.get("away")) and score_int(a.get("home_score"))==score_int(b.get("home_score")) and score_int(a.get("away_score"))==score_int(b.get("away_score")) and str(a.get("date") or "")==str(b.get("date") or "")
...
def merge(data, result):
    ...
    for club in aliases(result["home"])|aliases(result["away"]):
        ...
        conflicts=[x for x in rows if isinstance(x,dict) and pair_key(x.get("home"),x.get("away"))==pair_key(result["home"],result["away"]) and str(x.get("date") or "")==result["date"] and not same_result(x,result)]
        if conflicts: fail("merge_conflict",f"Replay reconciliation blocked: conflicting canonical replay for {result['home']} v {result['away']}",result=result,conflicts=conflicts)
        if not any(same_result(x,result) for x in rows if isinstance(x,dict)):
            rows.append(dict(result)); rows.sort(key=lambda x:(x.get("date",""),0 if "Replay" not in str(x.get("round","")) else 1)); changed=True
        current=(data.setdefault("results",{}) or {}).get(club)
        if not isinstance(current,dict) or not same_result(current,result): data["results"][club]=dict(result); changed=True
    return changed


def resolve_conditional_side(side,winners):
    text=str(side or ""); alts=[x.strip() for x in re.split(r"\s+or\s+",text,flags=re.I) if x.strip()]
    if len(alts)<=1:return text
    matches=list(dict.fromkeys(w for w in winners if sum(1 for alt in alts if compatible(alt,w))==1))
    ...
    for src in second_round_sources(data):
        for f in fixture_values(src):
            ...
            for field in ("home","away"):
                old=f.get(field,""); new=resolve_conditional_side(old,winners)
                if new!=old:f[field]=new;changed+=1
    return changed


def winner_in_next_round(data,winner):
    return any(isinstance(f,dict) and (compatible(f.get("home"),winner) or compatible(f.get("away"),winner)) for src in second_round_sources(data) for f in fixture_values(src))


def main():
    manifest_pairs={pair_key(x[1],x[4]) for x in VERIFIED_REPLAYS}
    ...
    if changed or resolved:
        data["updated_at"]=datetime.now(timezone.utc).isoformat(); DATA.write_text(json.dumps(data,indent=2,ensure_ascii=False)+"\n",encoding="utf-8")
    write_report("complete",status="PASS",archived_ties=len(ties),verified_replays=len(results),records_added_or_updated=changed,conditional_sides_resolved=resolved,replay_winners_linked=len(winners))
    print("FIRST QUALIFYING REPLAY RECONCILIATION: PASS"); print("Archived First Qualifying ties:",len(ties)); print("Verified replay manifest:",len(results)); print("Prerequisite drawn first legs verified:",len(results)); print("Replay records added/updated:",changed); print("Conditional Second Qualifying sides resolved:",resolved); print("Replay winners linked to Second Qualifying:",len(winners))
