...
requireIt(html.includes('.g-num{font-stretch:condensed;font-family:Arial Narrow,Arial,Helvetica,sans-serif;font-size:20pt;font-weight:700;line-height:1;white-space:nowrap}')||pmSlice.includes('style="white-space:nowrap"'),'Pigeon Miles value can wrap');
requireIt(html.includes("const TIN_FOIL_CHALLENGE_BRIDGE_KEY='tffc.clubfinderCampaign.v1';"),'Challenges bridge key missing');
requireIt(html.includes('beta/challenges-beta.html'),'production Challenges path incorrect');
requireIt(!html.includes('beta/beta/challenges-beta.html'),'production Challenges path double-prefixed');
requireIt(html.includes("window.name='TFFC_CLUBFINDER'"),'Clubfinder window identity missing');
requireIt(html.includes('TFFC_CHALLENGES'),'named Challenges window identity missing');
