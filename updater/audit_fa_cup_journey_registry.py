...
def key(n):
 s=str(n or '').lower().replace('&',' and '); s=re.sub(r'\b(association football club|football club)\b',' ',s); s=re.sub(r'\b(fc|afc|cfc)\b',' ',s); s=re.sub(r'[^a-z0-9]+',' ',s).strip(); return re.sub(r'\s+',' ',s)
def arr(t,n):
...
md=['# FA Cup Journey Registry — Read-only reconciliation','',f'- Official accepted clubs: **743**',f'- Protected origin records: **491**',f'- Reconciled official origin identities: **{len(covered)}**',f'- Additional journey clubs: **{len(missing)}**',f'- Existing ground records among additional clubs: **{existing}**',f'- Pending ground verification: **{pending}**','','## Identity reconciliations']+[f'- {a} → {b}' for a,b in rec]+['','## Merged origin identities']+([f"- {o} ← {', '.join(v)}" for o,v in merged.items()] or ['- None'])+['','## Entry-round population']+[f'- {r}: **{counts[r]}**' for r in ROUNDS if r in counts]+['','## Verification queue']+[f"- {x['club']} — {x['entry_round']}"+(f" — existing GROUNDS: {x['existing_ground_record']}" if x['existing_ground_record'] else '') for x in queue]+['','## Safety','- READ ONLY. Canonical Clubfinder, competition, grounds, mileage and journey data untouched.','- Explicit aliases only; ambiguous canonical collisions fail closed.']
