...
def fa_key(n):
 # Conservative competition identity: AFC is significant (AFC Liverpool != Liverpool FC).
 s=_base(n); s=re.sub(r'\b(fc|cfc)\b',' ',s); return re.sub(r'\s+',' ',s).strip()
def ground_key(n):
 # Compatibility identity for the already-verified origin↔GROUNDS relationship.
...
def eround(line):
 ...
 # Only accept actual exemption headings, never prose containing "exempt to".
 for r in ROUNDS:
  if re.fullmatch(rf'(?:CLUBS?\s+)?EXEMPT TO(?: THE)? {re.escape(r.upper())}(?:\s*\([^)]*\))?',u):return r
 return None
...
# Independent structural sanity: 219 EPR ties means 438 clubs enter EPR.
if counts.get('Extra Preliminary Round')!=438:raise SystemExit(f"ABORT: expected 438 Extra Preliminary entrants from official 219-tie draw, got {counts.get('Extra Preliminary Round')}")
...
print('FA CUP JOURNEY REGISTRY AUDIT: SUCCESS');print('Covered official identities:',len(covered));print('Additional:',len(missing));print('Existing ground records:',existing);print('Pending:',pending);print('Entry rounds:',counts);print('READ ONLY')