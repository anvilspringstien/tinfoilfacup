...
def exemption_variants(name):
 ...
 out.extend(EXEMPTION_DOCUMENT_ALIASES.get(name,[]))
...
# Existing GROUNDS lookup: prefer conservative FA identity. Relaxed ground matching is
# allowed only where the relaxed key maps to exactly one club in the official 743 field.
gby_fa=defaultdict(list); gby_relaxed=defaultdict(list); official_by_relaxed=defaultdict(list)
for n in gnames:
 gby_fa[fa_key(n)].append(n); gby_relaxed[ground_key(n)].append(n)
...
for c in missing:
 ...
 if exact:e=exact[0];method='fa-identity'
  ...
  relaxed=gby_relaxed.get(ground_key(c),[]); official_peers=official_by_relaxed.get(ground_key(c),[])
  if len(relaxed)>1:raise SystemExit(f'ABORT: ambiguous relaxed existing GROUNDS match for {c}: {relaxed}')
  if len(relaxed)==1 and len(official_peers)==1:e=relaxed[0];method='unique-ground-compatible'
 queue.append({'club':c,'entry_round':rm.get(fa_key(c),'Extra Preliminary Round'),'existing_ground_record':e,'ground_match_method':method,'verification_status':'existing-ground-record-needs-registry-review' if e else 'pending'})
existing=sum(bool(x['existing_ground_record']) for x in queue); pending=len(queue)-existing
report={'official_accepted':743,'protected_origin_records':491,'protected_origin_ground_matches':491,'reconciled_official_origin_identities':len(covered),'identity_reconciliations':[{'origin_name':a,'official_name':b} for a,b in rec],'exemption_document_aliases':EXEMPTION_DOCUMENT_ALIASES,'additional_journey_clubs':len(missing),'raw_ground_records':len(gnames),'additional_clubs_with_existing_ground_record':existing,'additional_clubs_pending_ground_verification':pending,'entry_round_counts':counts,'step5_promoted_epr_clubs':len(step5_promoted),'step4_lowest_ranked_epr_clubs':len(step4_lowest),'additional_clubs':queue,'read_only':True}
...
md=['# FA Cup Journey Registry — Read-only reconciliation','','- Official accepted clubs: **743**','- Protected origin records: **491**','- Protected origin→GROUNDS matches: **491**',f'- Reconciled official origin identities: **{len(covered)}**',f'- Additional journey clubs: **{len(missing)}**',f'- Existing ground records among additional clubs: **{existing}**',f'- Pending ground verification: **{pending}**','- Step 5 promoted clubs entering Extra Preliminary: **32**','- Step 4 lowest-ranked clubs entering Extra Preliminary: **91**','','## Identity reconciliations']+[f'- {a} → {b}' for a,b in rec]+['','## Entry-round population']+[f'- {r}: **{counts[r]}**' for r in ROUNDS if r in counts]+['','## Verification queue']+[f"- {x['club']} — {x['entry_round']}"+(f" — existing GROUNDS: {x['existing_ground_record']} ({x['ground_match_method']})" if x['existing_ground_record'] else '') for x in queue]+['','## Safety','- READ ONLY. Canonical Clubfinder, competition, grounds, mileage and journey data untouched.','- Separate FA identity and verified-ground identity namespaces.','- Leading AFC is identity-significant; trailing AFC/FC suffix variants reconcile only through a unique official identity.','- Bedfont Sports Club FC ↔ Bedfont Sports FC is an explicit FA-document alias, not a generic name rule.','- Relaxed later-round GROUNDS matching is forbidden when more than one official FA identity shares that relaxed key.','- Exact 491 + 252 = 743 partition required.','- Entry-round totals cross-checked against official headings and the 219-tie Extra Preliminary draw.','- The Step 4 exception is independently validated as 32 promoted Step 5 + 91 lowest-ranked Step 4 = 123 clubs using layout-mode PDF extraction and full-name boundaries.']
