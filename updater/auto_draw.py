...
The official FA fixtures endpoint is authoritative for publication detection.
Only the round immediately after competition.json/source_round can be promoted.
...
UA = {"User-Agent": "Mozilla/5.0 TinFoilFACupDrawWatcher/1.0", "Accept": "text/html,application/xhtml+xml"}
...
def round_from_context(context):
    text = clean(context)
    best = None
    for canonical, aliases in ROUND_ALIASES.items():
...
def validate_target(target, ties):
    if len(ties) < 20 and target not in {"Quarter Final", "Semi Final", "Final"}:
        raise SystemExit(f"Publication blocked: only {len(ties)} {target} ties found on official FA fixture pages.")
    ...
    dated = [f["date"] for f in ties if f.get("date")]
    if not dated:
...
def main():
    ...
    if idx + 1 >= len(ROUND_ORDER):
        print("No later FA Cup round exists after", current)
        return
    ...
    for page in range(1, args.max_pages + 1):
        html = fetch(FA_PAGE.format(page))
        pages_checked += 1
        rows = parse_page(html)
        all_rows.extend(rows)
        # Once target rows have been seen and this page contains a later round,
        # there is no need to continue paginating.
        rounds = {r["round"] for r in rows}
        if target in {r["round"] for r in all_rows} and any(ROUND_ORDER.index(r) > ROUND_ORDER.index(target) for r in rounds if r in ROUND_ORDER):
            break
    ...
    if not target_ties:
        print(f"NO NEW DRAW: official FA fixture pages do not yet expose {target}.")
        return

    validate_target(target, target_ties)
    print(f"OFFICIAL DRAW DETECTED: {target}: {len(target_ties)} ties")
    if not args.publish:
    ...
    data["source_url"] = "https://www.thefa.com/competitions/thefacup/fixtures"
    data["updated_at"] = datetime.now(timezone.utc).isoformat()
    DATA.write_text(json.dumps(data, indent=2, ensure_ascii=False) + "\n")

    report["published"] = True
    report["archived_outgoing_round"] = current
    report["archived_outgoing_ties"] = len(old)
    REPORT.write_text(json.dumps(report, indent=2) + "\n")
