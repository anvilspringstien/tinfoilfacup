...
if not HTML.exists() or not QUEUE.exists():
    raise SystemExit("Missing clubfinder.html or v7.9.1 correction queue.")
...
# Coordinates are Open Postcode Geo postcode-centroid coordinates, used here only
# as deterministic candidate coordinates for postcode-changing corrections.
TARGETS=[
...
s,e=locate(text,"GROUNDS")
grounds=json.loads(text[s:e])
rows=[]
for t in TARGETS:
    matches=[g for g in grounds if norm(g.get("name") or g.get("club"))==norm(t["club"])]
    if len(matches)!=1:
        raise SystemExit(f"Safety stop: expected exactly one canonical GROUNDS record for {t['club']}; found {len(matches)}")
    g=matches[0]
    current={k:g.get(k) for k in ("name","club","ground","postcode","lat","lon","verification","verification_label","source") if k in g}
    current_pc=(g.get("postcode") or "").upper().strip()
    target_pc=t["postcode"]
    rows.append({
      **t,
      "current":current,
      "postcode_change":current_pc!=target_pc,
      "ground_change":norm(g.get("ground"))!=norm(t["ground"]),
      "target_coordinates_kind":"postcode_centroid_candidate",
      "publish_ready_after_human_confirmation":True,
    })

if len(rows)!=7:
    raise SystemExit("Safety stop: validation must contain exactly seven records.")
now=datetime.now(timezone.utc)
payload={
 "checked_at":now.isoformat(),"version":"7.9.2","mode":"VALIDATION ONLY / NO CANONICAL WRITE",
 "source_queue_version":"7.9.1","records_checked":7,"canonical_records_changed":0,
 "held_relationships_unchanged":1,"items":rows,"held":held,
}
OUT.write_text(json.dumps(payload,indent=2,ensure_ascii=False)+"\n",encoding="utf8")

L=["# Tin Foil FA Cup — Current Venue Validation","",f"Last checked: **{now.strftime('%d/%m/%Y, %H:%M:%S UTC')}**","","**v7.9.2 VALIDATION ONLY. No canonical GROUNDS record, approval ledger, `clubfinder.html`, or `competition.json` is changed.**","","- Canonical records checked: **7**","- Canonical records changed: **0**","- Held relationships changed: **0**","","## Canonical comparison",""]
for r in rows:
    c=r["current"]
    L += [f"### {r['club']}","",f"- Action: `{r['action']}`",f"- Current canonical: **{c.get('ground','(blank)')} • {c.get('postcode','(blank)')}** • `{c.get('lat','?')}, {c.get('lon','?')}`",f"- Validated target: **{r['ground']} • {r['postcode']}** • `{r['lat']}, {r['lon']}`",f"- Postcode change required: **{'YES' if r['postcode_change'] else 'NO'}**",f"- Ground-name change required: **{'YES' if r['ground_change'] else 'NO'}**",f"- Venue evidence: {r['source']}",f"- Coordinate basis: {r['coordinate_source']} (postcode centroid candidate)",""]
L += ["## Held outside scope","","- **Romulus FC ↔ Sutton Coldfield Town FC** remains `HELD_FOR_MORE_EVIDENCE` and is not validated, corrected, approved or published by this stage.","","## Safety","","- Exactly one live `GROUNDS` record must exist for every one of the seven named clubs or the run stops.","- v7.9.1 must still contain exactly seven queue units and zero published canonical records.","- The three stale relationships #8/#10/#14 are not revived.","- Coordinates in this report are deterministic postcode-centroid candidates, not silently substituted for existing exact ground coordinates when the postcode is unchanged.","- This stage writes only the validation JSON/report."]
REPORT.write_text("\n".join(L)+"\n",encoding="utf8")
print("CURRENT VENUE VALIDATION v7.9.2")
print("Canonical records checked: 7")
print("Canonical records changed: 0")
for r in rows:
    c=r['current']; print(f"{r['club']}: {c.get('ground')} / {c.get('postcode')} -> {r['ground']} / {r['postcode']} | postcode_change={r['postcode_change']}")
print("Romulus/Sutton Coldfield: HELD, untouched")
print("VALIDATION ONLY / NO CANONICAL WRITE")
