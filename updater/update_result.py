#!/usr/bin/env python3
import json,re,sys
from datetime import datetime,timezone
from pathlib import Path

ROOT=Path(__file__).resolve().parents[1]
P=ROOT/'competition.json'

if len(sys.argv)<8:
    raise SystemExit('Usage: update_result.py HOME AWAY HS AS ROUND DATE SOURCE [DECISION]')

home,away=sys.argv[1].strip(),sys.argv[2].strip()
hs,as_=int(sys.argv[3]),int(sys.argv[4])
rnd,date,src=sys.argv[5].strip(),sys.argv[6].strip(),sys.argv[7].strip()
decision=sys.argv[8].strip() if len(sys.argv)>8 else ''

if hs<0 or as_<0: raise SystemExit('Negative score invalid')
if hs==as_ and not decision: raise SystemExit('Level result needs decision')

winner=home if hs>as_ else away if as_>hs else ''
if hs==as_ and decision.lower()!='draw-replay':
    m=re.search(r'winner\s*:\s*([^;]+)',decision,re.I)
    if m: winner=m.group(1).strip()

result={
  'home':home,'away':away,'home_score':hs,'away_score':as_,
  'winner':winner,'status':'FT','decision':decision,
  'date':date,'round':rnd,'source_url':src
}

data=json.loads(P.read_text(encoding='utf-8'))
results=data.setdefault('results',{})
history=data.setdefault('result_history',{})
replays=data.setdefault('replays',{})
suffix=re.compile(r'\s+(FC|AFC|CFC)$',re.I)

def aliases(n):
    s={n,suffix.sub('',n)}
    if not suffix.search(n): s|={n+' FC',n+' AFC'}
    return {x for x in s if x}

def same(a,b):
    return (
      str(a.get('home','')).lower()==str(b.get('home','')).lower() and
      str(a.get('away','')).lower()==str(b.get('away','')).lower() and
      str(a.get('date',''))==str(b.get('date','')) and
      str(a.get('home_score',''))==str(b.get('home_score','')) and
      str(a.get('away_score',''))==str(b.get('away_score',''))
    )

all_aliases=aliases(home)|aliases(away)

# Append this match to chronological history for both participants.
for club in all_aliases:
    arr=history.setdefault(club,[])
    if not any(same(x,result) for x in arr):
        arr.append(result)
    arr.sort(key=lambda x:x.get('date',''))
    results[club]=result

# A completed non-draw replay supersedes replay-pending state.
if decision.lower()!='draw-replay' and winner:
    shorts={suffix.sub('',home),suffix.sub('',away)}
    for club in list(replays):
        if suffix.sub('',club) in shorts:
            replays.pop(club,None)

data['updated_at']=datetime.now(timezone.utc).isoformat()
data['last_result_source_url']=src
data['last_result_round']=rnd
data['last_result']={
  'home':home,'away':away,'home_score':hs,'away_score':as_,
  'winner':winner,'date':date
}

tmp=P.with_suffix('.json.new')
tmp.write_text(json.dumps(data,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
tmp.replace(P)

print(f'SUCCESS: {home} {hs}-{as_} {away}')
print('Winner:',winner or 'No winner / replay required')
print('History entries for home:',len(history.get(home,[])))
print('Fixtures preserved:',len(data.get('fixtures',{})))
