...
TARGETS=[
 {"name":"Liskeard Athletic FC","ground":"Lux Park","postcode":"PL14 3HZ","lat":50.460916,"lon":-4.46164,"source":"https://www.swpleague.co.uk/div-1-west/division-1-west/","ground_source":"Current league club directory; Lux Park independently confirmed","coordinate_source":"FCHD mapped Lux Park ground coordinate","eligible_required":True},
 {"name":"Stockport Georgians FC","ground":"Cromley Road","postcode":"SK2 7DT","lat":53.3831,"lon":-2.1473,"source":"https://www.pitchero.com/clubs/stockportgeorgiansfc/contact","ground_source":"Current official club contact page","coordinate_source":"Open postcode/playing-field geodata for SK2 7DT; football playing field independently listed at address","eligible_required":True},
 {"name":"SE Dons FC","ground":"The Bauvill Stadium","postcode":"ME4 6LR","lat":51.3686,"lon":0.5214,"source":"https://fulltime.thefa.com/displayTeam.html?divisionseason=261605801&teamID=80521829","ground_source":"FA Full-Time and current SCEFL evidence place SE Dons home matches at Bauvill Stadium","coordinate_source":"Mapped Chatham Town/Bauvill Stadium ground coordinate","eligible_required":True},
 {"name":"Chatham Town FC","ground":"The Bauvill Stadium","postcode":"ME4 6LR","lat":51.3686,"lon":0.5214,"source":"https://www.chathamtownfc.com/contact-us/","ground_source":"Current official Chatham Town contact page","coordinate_source":"Mapped Bauvill Stadium ground coordinate","eligible_required":False},
]
REL=[
...
for t in TARGETS:
 ...
 if t['eligible_required'] and k not in elig: raise SystemExit(f"Safety stop: {t['name']} not eligible.")
 if by.get(k): raise SystemExit(f"Safety stop: existing canonical record for {t['name']}.")
 canonical_name=elig.get(k,t['name'])
 new.append({"name":canonical_name,"ground":t['ground'],"postcode":t['postcode'],"lat":t['lat'],"lon":t['lon'],"verification":"verified","verification_label":"✅ Verified","source":"Current 2026/27 venue evidence: "+t['source'],"ground_source":t['ground_source'],"coordinate_source":t['coordinate_source']})
if len(new)!=4: raise SystemExit('Exactly four new records required, including Chatham Town host.')
...
payload={"published_at":now,"version":"7.9.10","canonical_records_added":4,"eligible_records_added":3,"host_records_added":1,"existing_records_overwritten":0,"groundshares_recorded":2,"records":new,"relationships":REL,"protected_held":["Romulus FC","Sutton Coldfield Town FC"],"competition_json_changed":False}; OUT.write_text(json.dumps(payload,indent=2,ensure_ascii=False)+'\n',encoding='utf8')
lines=['# Tin Foil FA Cup — Critical Ground Batch 5','',f'Published: **{datetime.now(timezone.utc).strftime("%d/%m/%Y, %H:%M:%S UTC")}**','','- New verified eligible canonical records: **3**','- New verified host canonical records: **1**','- Existing records overwritten: **0**','- Current groundshares recorded: **2**','- Protected Romulus/Sutton Coldfield records changed: **0**','- `competition.json` changed: **NO**','','## Published records','']
...
REPORT.write_text('\n'.join(lines)+'\n',encoding='utf8')
print('CRITICAL GROUND BATCH 5 v7.9.10'); print('Eligible canonical records: 3'); print('Host canonical records: 1'); print('Groundshares recorded: 2'); print('Existing records overwritten: 0'); print('Romulus/Sutton Coldfield: untouched')
