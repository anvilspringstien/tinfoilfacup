...
for (const [oldText, newText] of edits) {
  ...
  if (occurrences !== 1) throw Error('Unsafe patch: expected one occurrence, found ' + occurrences + ': ' + oldText.slice(0, 70));
  html = html.replace(oldText, newText);
}
fs.writeFileSync(path, html);
console.log('Stats wide-viewport patches applied exactly once; competition and production untouched');
