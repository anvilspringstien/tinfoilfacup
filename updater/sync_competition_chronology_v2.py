...
def enrich_kickoffs(results,html):
    """Attach local kick-off times from a static competition listing without altering scores/results."""
    ...
    for r in results:
        ...
        m=pat.search(text)
        if not m:
            pat=re.compile(r'(\d{1,2}:\d{2})\s+'+hp+r'.{0,60}?'+ap,re.I)
            m=pat.search(text)
        if m:
            r['kickoff']=parse_time(m.group(1))
            r['kickoff_source_url']=KICKOFF_SOURCE_URL
            enriched+=1
    return enriched
...
data['competition_sync']={'source':'Football Web Pages + static kick-off listing','source_url':FIXTURES_URL,'kickoff_source_url':KICKOFF_SOURCE_URL,'synced_at':data['updated_at'],'preliminary_fixture_source':'played original ties','preliminary_unique_fixtures':prelim_count,'preliminary_results_and_replays':result_count,'first_qualifying_unique_fixtures':len(frq),'result_dates':RESULT_DATES,'kickoffs_enriched':kickoff_enriched,'replay_kickoffs_verified':len(replay_results)}
