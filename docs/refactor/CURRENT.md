...
**Last verified:** 25 September 2026 (BST), after completed and green **BETA Deck renderer Stage B PR #105**, independently protected by baseline test-only PR #106; [structural plan](./2026-09-25-bridge-render-structural-plan.md) remains the staged roadmap.  
...
Take **Stage C** from the [structural plan](./2026-09-25-bridge-render-structural-plan.md) as a **read-only** full-file BETA Clubfinder producer audit first. Its 4.6 MB source must be inspected in-repository rather than via truncated connector responses. Map `openChallenges(origin)`, `tinFoilChallengeStatsSnapshot`, identity save/recovery, return-snapshot restoration and canonical venue-based Pigeon Miles calculations to the new Stage A reader. Before **any** BETA Clubfinder edits, implement an independent guarded workflow allowing that file but still protecting production and competition data, with full Clubfinder VM/mobile/venue/replay/Deck bridge tests. Do not start a shared module or production promotion without necessity and independent acceptance. Continue separate proved-dead CSS investigations and standalone offline-snapshot freshness work only in their own guarded PRs.
