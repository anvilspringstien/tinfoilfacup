...
## Reviewed Stage 1 findings (full-source audit and targeted call inspection)

- The generated [full-source map](./2026-09-25-generated-source-map.md) reads every checked-out source file on a GitHub Actions runner and records reproducible SHA-256 fingerprints. BETA Clubfinder is **4,616,151 bytes** and BETA Challenges **88,761 bytes** at the audit commit; the inventory finds **150** distinct named BETA Clubfinder functions and **30** in Challenges. These metrics identify analysis scope, **not** safely removable code or guaranteed performance savings.
- **Do not delete `renderStats`.** Inspection of the accepted Challenge Deck shows it is actively called by `save()`, Accepted/Completed actions, simulator changes and startup. Despite its historic name, it populates the new campaign-truth frame (Call Sign, Pigeon Name, Pigeon Miles Flown, current round), simulator counters/status and the Trophy Cabinet, including click-to-inspect earned mats. Removing it on the assumption that detached Challenges Stats was deleted would break accepted functionality.
- The removed splash **markup is absent**, but the Challenges file still retains numerous `.beta-splash` styles and `#maybeLater` CSS selectors. No matching splash markup or script references were found in targeted source inspection. Their CSS block is a promising **first isolated cleanup candidate**, conditional on a complete selector/cascade check and green direct-entry/Trophy/phone regression tests; do not touch the active state logic in the same PR.
- Full-file audit recorded the offline fallback as **STALE** after the normal live chronology update: embedded `2026-09-25T07:49:01.065159+00:00` versus checked-out canonical `2026-09-25T12:45:37.316822+00:00`. Online canonical fetch remains authoritative. Schedule a **separate guarded fallback refresh** before any PR that requires the strict current-canonical BETA smoke gate; no competition-data rewrite in the refactor branch.
- Stage 1 inventory workflow runs the standalone Deck and Trophy regressions and the accepted mobile-name patcher `--check`, verifies report reproducibility and checks the four protected HTML/data files against immutable starting commit `1ab8276` **before** it may commit the generated document. No user-facing BETA changes are part of this work.
